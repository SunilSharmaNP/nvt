...
class Database:
    ...
    async def update_user_settings(self, user_id: int, settings: Dict):
        """Update user settings - FIXED for proper persistence"""
        await self.users.update_one(
            {"user_id": user_id},
    ...
    async def update_user_setting(self, user_id: int, key: str, value):
        """Update a single setting - FIXED"""
        await self.users.update_one(
            {"user_id": user_id},
            {"$set": {f"settings.{key}": value, "last_used": datetime.utcnow()}}
        )
    ...
    async def set_encoding_settings(self, user_id: int, settings: Dict):
        """Set encoding settings for user - FIXED"""
        await self.users.update_one(
            {"user_id": user_id},
    ...
    async def set_sample_duration(self, user_id: int, duration: str):
    ...
    async def add_to_merge_queue(self, user_id: int, file_info: Dict):
        """Add file to merge queue"""
        await self.users.update_one(
            {"user_id": user_id},
            {"$push": {"merge_queue": file_info}}
        )
    
    async def get_merge_queue(self, user_id: int) -> List[Dict]:
        """Get user's merge queue"""
        user = await self.get_user(user_id)
        return user.get("merge_queue", []) if user else []
    
    async def clear_merge_queue(self, user_id: int):
